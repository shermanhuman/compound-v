<!-- Auto-generated by promptherder — do not edit -->
...
6. `/stack` — discover and manage pinned versions
...
Project-level rules that are always active.

- **Releases: semver.** Tag format is `vX.Y.Z`. Version lives in `herd.json` and is bumped before opening PRs. Releases are automated: when a PR with version bump is merged, CI creates the tag and publishes. Bump minor for new features. Bump patch for fixes and improvements. Do not bump major without explicit approval — major is reserved for a stability milestone (v1.0.0).

- **Merging is a human task.** Never merge a branch, squash-merge a PR, or push directly to main. You may create branches, push to feature branches, and create PRs. When work is ready to merge, ask the user to merge. This rule has no exceptions.
